...
class GmailAIAgent:
    ...
    def delete_emails_from_sender_duration_ago(self, sender: str, duration_ago: str, confirm: bool = False):
        """Delete emails from a specific sender within a specific duration-ago window.
        Example: sender='google', duration_ago='5 months ago' → query "from:google after:YYYY/MM/DD before:YYYY/MM/DD".
        """
        try:
            if not sender or not duration_ago:
                return {"status": "error", "message": "Missing sender or duration_ago"}

            # Detect Hebrew UI for localized messages
            try:
                from django.utils import translation as _translation
                lang_code = _translation.get_language() or 'en'
            except Exception:
            ...
            def _hebrew_duration_phrase(eng_phrase: str) -> str:
                ...
                import re
                if 'month' in eng_lower and 'ago' in eng_lower:
                    m = re.search(r'(\d+|\w+)\s+months?', eng_lower)
                    if m:
                        n = m.group(1)
                        if str(n).isdigit():
                            return f"לפני {n} חודשים"
                        if str(n).lower() in ['a', 'one']:
                            return "לפני חודש"
                if 'week' in eng_lower and 'ago' in eng_lower:
                    m = re.search(r'(\d+|\w+)\s+weeks?', eng_lower)
                    if m:
                        n = m.group(1)
                        if str(n).isdigit():
                            return f"לפני {n} שבועות"
                        if str(n).lower() in ['a', 'one']:
                            return "לפני שבוע"
                if 'year' in eng_lower and 'ago' in eng_lower:
                    m = re.search(r'(\d+|\w+)\s+years?', eng_lower)
                    if m:
                        n = m.group(1)
                        if str(n).isdigit():
                            return f"לפני {n} שנים"
                        if str(n).lower() in ['a', 'one']:
                            return "לפני שנה"
                if 'day' in eng_lower and 'ago' in eng_lower:
                    m = re.search(r'(\d+|\w+)\s+days?', eng_lower)
                    if m:
                        n = m.group(1)
                        if str(n).isdigit():
                            return f"לפני {n} ימים"
                        if str(n).lower() in ['a', 'one']:
                            return "לפני יום"
                return eng_phrase
            ...
            if not all_messages:
                if hebrew_mode:
                    return {"status": "success", "message": f"לא נמצאו מיילים מ{sender} {display_duration} למחיקה.", "deleted_count": 0}
                return {"status": "success", "message": _("No emails found from %(sender)s %(duration)s to delete.") % {"sender": sender, "duration": display_duration}, "deleted_count": 0}

            if not confirm:
                if hebrew_mode:
                    confirm_msg = f"נמצאו {len(all_messages)} מיילים מ{sender} {display_duration}. האם להעביר לאשפה?"
                else:
                    confirm_msg = _("Found %(count)d emails from %(sender)s %(duration)s. Move to Trash?") % {"count": len(all_messages), "sender": sender, "duration": display_duration}
                return {
                    "status": "confirmation_required",
                    "message": confirm_msg,
                    "count": len(all_messages),
                    "total_estimated": len(all_messages),
                    "preview": self._build_preview(all_messages),
                    "action_details": {"action": "delete_by_sender_duration_ago", "sender": sender, "duration_ago": duration_ago}
                }
            ...
            action_id = self._record_undo('trash', message_ids)
            if hebrew_mode:
                return {"status": "success", "message": f"הועברו לאשפה {total_processed} מיילים מ{sender} {display_duration}.", "deleted_count": total_processed, "undo_action_id": action_id}
            return {"status": "success", "message": _("Trashed %(count)d emails from %(sender)s %(duration)s") % {"count": total_processed, "sender": sender, "duration": display_duration}, "deleted_count": total_processed, "undo_action_id": action_id}
            ...
            if str(lang_code).startswith('he'):
                return {"status": "error", "message": f"שגיאה במחיקת מיילים מ{sender} {duration_ago}: {err_msg}"}
            return {"status": "error", "message": _("Error deleting emails from %(sender)s %(duration)s: %(error)s") % {"sender": sender, "duration": duration_ago, "error": err_msg}}
    ...
    def parse_command_manually(self, command):
        ...
        if "empty trash" in command_lower:
            ...
            print(f"DEBUG: Action heuristic chose 'archive' for command: {command_lower}")
        else:
        ...
        # IMPORTANT: Only apply this fallback when the overall intent is 'list'.
        # Otherwise it can hijack 'archive' commands that also contain these tokens.
        if best_match_action == "list":
        ...
        if best_match_action == "delete":
            ...
            # "delete emails from [timeframe]" (today/yesterday/this|last week/month/year)
            time_match = re.search(r'(^|\b)delete(?:\s+emails?)?\s+from\s+(today|yesterday|this\s+week|last\s+week|this\s+month|last\s+month|this\s+year|last\s+year)(\b|$)', command_lower)
            if time_match:
            ...
            if sender_duration_ago_match:
                sender_kw = sender_duration_ago_match.group(2).strip()
                duration_ago = sender_duration_ago_match.group(3)
                return {"action": "delete", "target_type": "sender_duration_ago", "sender": sender_kw, "duration_ago": duration_ago, "confirmation_required": True}
        ...
        if best_match_action == "archive":
            ...
            if plain_time_match:
                # Group 2 is the actual timeframe (group 1/3 are word boundaries)
    ...
    def delete_emails_from_time(self, time_period, confirm=False):
        ...
        This moves any messages within the time window to Trash.
        """
        try:
            if not time_period:
                return {"status": "error", "message": "Missing required parameter: time_period"}
            ...
            print(f"DEBUG: delete_emails_from_time called with time_period='{time_period}'")
            start_dt, end_dt = self._compute_precise_date_range_window(str(time_period).strip().lower())
            if not start_dt or not end_dt:
                return {"status": "error", "message": f"Invalid or unsupported time period: {time_period}"}
            ...
            # Fetch all message ids with retries
            max_retries = 5
            retry_delay = 1
            all_messages = []
            next_page_token = None
            for attempt in range(max_retries):
                try:
                    ...
                    if (("SSL" in err_text or "WRONG_VERSION_NUMBER" in err_text or "DECRYPTION_FAILED_OR_BAD_RECORD_MAC" in err_text or
                         "timeout" in err_text.lower() or "connection" in err_text.lower() or
                         "WinError 10060" in err_text or "failed to respond" in err_text.lower()) and attempt < max_retries - 1):
                        print(f"Connection error in delete_emails_from_time (attempt {attempt + 1}): {err_text}")
                        time.sleep(retry_delay)
                        retry_delay *= 2
                        continue
                    else:
            ...
            if not all_messages:
                if hebrew_mode:
                    return {"status": "success", "message": f"לא נמצאו מיילים {display_time} למחיקה.", "deleted_count": 0}
                return {"status": "success", "message": _("No emails found from %(period)s to delete.") % {"period": display_time}, "deleted_count": 0}

            if not confirm:
                if hebrew_mode:
                    confirm_msg = f"נמצאו {len(all_messages)} מיילים {display_time}. האם להעביר אותם לאשפה?"
                else:
                    confirm_msg = _("Found %(count)d emails from %(period)s. Do you want to move them to Trash?") % {"count": len(all_messages), "period": display_time}
                return {
                    "status": "confirmation_required",
                    "message": confirm_msg,
                    "count": len(all_messages),
                    "total_estimated": len(all_messages),
                    "preview": self._build_preview(all_messages),
                    "action_details": {"action": "delete_by_time", "time_period": time_period}
                }
            ...
            action_id = self._record_undo('trash', message_ids)
            if hebrew_mode:
                return {"status": "success", "message": f"הועברו לאשפה {total_processed} מיילים {display_time}.", "deleted_count": total_processed, "undo_action_id": action_id}
            return {"status": "success", "message": _("Trashed %(count)d emails from %(period)s") % {"count": total_processed, "period": display_time}, "deleted_count": total_processed, "undo_action_id": action_id}
            ...
            try:
                ...
                lang_code = 'en'
            if str(lang_code).startswith('he'):
                return {"status": "error", "message": f"שגיאה במחיקת מיילים {time_period}: {err_msg}"}
            return {"status": "error", "message": _("Error deleting emails from %(period)s: %(error)s") % {"period": time_period, "error": err_msg}}
    ...
    def archive_emails_from_time(self, time_period, confirm=False):
        ...
        try:
            if not time_period:
                return {"error": "Missing required parameter: time_period"}
            ...
            if not confirm:
                return {
                    "status": "confirmation_required",
                    "message": _("Found %(count)d emails from %(time)s. Do you want to archive them?") % {"count": len(all_messages), "time": display_time},
                    "count": len(all_messages),
                    "total_estimated": len(all_messages),
                    "preview": self._build_preview(all_messages),
                    "action_details": {"action": "archive_by_time", "time_period": time_period}
                }
